...
3. **AI 맞춤 추천**을 누르고 대화형 질문에 따라 동행, 분위기, 활동, 지역, 경험, 수영 여부, 예산과 나이를 답합니다.
4. 대화 안에서 Solar가 추천한 상품 3개와 추천 이유를 확인한 뒤 상품 상세나 전체 추천 목록으로 이동합니다.
5. 상품 카드를 눌러 예약과 전자서명 흐름을 체험합니다.
6. 웹사이트 안의 모두싸인 화면에서 서명합니다. 한국어 외 언어를 선택한 구매자는 오른쪽의 **계약서 번역** 탭에서 실제 계약서의 AI 번역을 참고할 수 있으며, 전자서명은 왼쪽 원문을 기준으로 진행됩니다. 서명이 끝나면 완료 문서가 회원가입 이메일로 전달됩니다.
...
12. 구매자는 예약 화면에서 미성년 여부를 반드시 선택합니다. 판매자의 상품을 예약하면 판매자 센터 상단에 새 예약 알림이 표시되며, **내 예약** 카드의 **초안 작성**, **계약서 검토·수정**, **최종 서명 요청**, **예약 취소** 버튼으로 각 작업을 진행할 수 있습니다. **초안 작성**은 새 초안을 시작하거나 작성 중인 초안을 다시 열 때 모두 사용할 수 있습니다. 카드 왼쪽 위에는 현재 계약 단계가, 오른쪽 위에는 최종 수정 시각이 표시되며 **새로고침**으로 최신 상태를 다시 확인할 수 있습니다.
13. 초안 작성 화면에서 **AI 추천 계약서**를 선택하면 AI 추천 목록에서 체크된 계약서를 사용하고, **템플릿 직접 선택**을 선택하면 전체 10개 템플릿 목록에서 판매자가 체크한 계약서를 사용합니다. **계약서 직접 작성**을 선택하면 빈 계약서에 제목, 이용 조건, 환불 규정, 안전·책임 범위와 특약을 직접 작성할 수 있습니다. 미성년자 법정대리인 동의서는 구매자가 미성년자라고 선택한 예약에만 필수가 되며, 예약·개인정보·기본 안전·환불처럼 규칙상 필수인 계약서는 화면과 서버 양쪽에서 해제할 수 없습니다. Solar 연결 실패 시에도 기본 안전 규칙 추천이 표시됩니다.
14. 템플릿 방식에서는 **선택 내용 저장**을 누르면 선택한 템플릿을 초안에 저장하고 예약 카드로 돌아갑니다. 직접 작성 방식에서는 계약 내용을 저장한 뒤 카드의 **계약서 검토·수정**에서 다시 확인할 수 있습니다. 이후 **최종 서명 요청**에서 저장된 계약서를 구매자에게 발송합니다. 최종 발송 전 모두싸인 화면에서 실제 문서와 서명 위치를 확인하며, 예약자 이름과 이메일은 자동 지정됩니다.
