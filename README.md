...
12. 구매자는 예약 화면에서 미성년 여부를 반드시 선택합니다. 판매자의 상품을 예약하면 판매자 센터 상단에 새 예약 알림이 표시되며, **들어온 예약**에서 **계약 초안 만들기**를 누른 뒤 `data/contract-templates.json`에 등록된 모두싸인 템플릿 또는 상품 기본 계약서 묶음을 선택합니다.
13. **AI 계약서 추천받기**를 누르면 상품 카테고리, 위험도, 장비·선박·잠수 여부, 날씨 영향과 예약자의 미성년 여부를 바탕으로 등록된 템플릿의 조합과 이유를 추천합니다. 미성년자 법정대리인 동의서는 구매자가 미성년자라고 선택한 예약에만 추천됩니다. 예약·개인정보·기본 안전·환불처럼 규칙상 필수인 계약서는 AI가 해제할 수 없으며, Solar 연결 실패 시에도 기본 안전 규칙 추천이 표시됩니다. 추천 결과는 상품 수정 시점과 미성년 여부가 같을 때 서버에서 재사용합니다.
14. 계약 화면은 **작성 방식 → 검토·수정 → 서명 요청** 순서로 진행됩니다. 판매자는 모두싸인 템플릿·AI 추천으로 시작하거나 **직접 작성**을 선택해 제목, 이용 조건, 환불 규정, 안전·책임 범위와 특약을 빈 초안에서 작성할 수 있습니다. 직접 작성 화면은 예약 기본정보를 자동 연결하고 실시간 미리보기를 제공하며, 상품의 기본 약관을 불러와 이어서 수정할 수도 있습니다. 최종 발송 전 모두싸인 화면에서 실제 문서와 서명 위치를 확인하며, 예약자 이름과 이메일은 자동 지정됩니다.
